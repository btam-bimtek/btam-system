...
// Data dari exam_submissions.warningCount + flagged + submitReason
...
export async function renderSubPelanggaran(container, bimtekId) {
  ...
  try {
    ...
    // Urutkan: flagged dulu, lalu berdasarkan warningCount desc
    submissions.sort((a, b) => {
      if (b.flagged !== a.flagged) return b.flagged ? 1 : -1;
      return (b.warningCount || 0) - (a.warningCount || 0);
    });

    const maxWarn = 3; // Sync dengan anti-cheat maxWarnings default

    container.innerHTML = `
      <div class="mb-4 flex gap-4 text-xs">
        <div class="bg-red-900 bg-opacity-30 px-3 py-2 rounded">
          <span class="text-red-400 font-bold">${submissions.filter(s => s.flagged).length}</span>
          <span class="text-red-300 ml-1">Auto-Submit</span>
        </div>
        <div class="bg-yellow-900 bg-opacity-30 px-3 py-2 rounded">
          <span class="text-yellow-400 font-bold">${submissions.filter(s => (s.warningCount || 0) > 0).length}</span>
        ...
        <table class="btam-table">
          ...
          <tbody>
            ${submissions.map(s => {
              ...
              const warnClass = isFlagged ? 'text-red-400 font-bold' : warn > 0 ? 'text-yellow-400' : 'text-gray-500';
              const reasonLabel = SUBMIT_REASON_LABEL[s.submitReason] || s.submitReason || '—';
              return `
                <tr class="${isFlagged ? 'bg-red-950 bg-opacity-20' : ''}">
                  ...
                  <td class="text-center text-xs text-gray-400">${s.tipeSession === 'pretest' ? 'Pre' : s.tipeSession === 'posttest' ? 'Post' : (s.tipeSession || '—')}</td>
